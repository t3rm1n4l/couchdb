...
test() ->
    ...
    {StreamReq0, {failoverlog, InitialFailoverLog0}} = couch_upr_client:add_stream(Pid, 0,
        lists:nth(1, InitialFailoverLog0), 10, 100),

    {StreamReq1, {failoverlog, InitialFailoverLog1}} = couch_upr_client:add_stream(Pid, 1,
        lists:nth(1, InitialFailoverLog1), 100, 200),

    {StreamReq2, {failoverlog, InitialFailoverLog2}} = couch_upr_client:add_stream(Pid, 2,
        lists:nth(1, InitialFailoverLog2), 0, 10),

    [MutationsPart0, MutationsPart1, MutationsPart2] = read_mutations(
                    Pid, [StreamReq0, StreamReq1, StreamReq2], [[], [], []]),


    etap:is(all_same_partition(0, MutationsPart0), true,
        "Stream0 has all partition0 mutations"),
    etap:is(all_same_partition(1, MutationsPart1), true,
        "Stream1 has all partition1 mutations"),
    etap:is(all_same_partition(2, MutationsPart2), true,
        "Stream2 has all partition2 mutations"),
    ...
    {StreamReq0_2, {failoverlog, InitialFailoverLog0}} = couch_upr_client:add_stream(Pid, 0,
        lists:nth(1, InitialFailoverLog0), 1, 100),

    {StreamReq0_3, StreamResp0_3} = couch_upr_client:add_stream(Pid, 0,
        lists:nth(1, InitialFailoverLog0), 10, 100),
    ...
    {StreamReq1_2, _} = couch_upr_client:add_stream(Pid, 1,
        lists:nth(1, InitialFailoverLog1), 10, 300),

    {StreamReq2_2, _} = couch_upr_client:add_stream(Pid, 2,
        lists:nth(1, InitialFailoverLog2), 100, 200),
...
all_same_partition(PartId, Docs) ->
...
read_mutations(Pid, StreamReqs, Acc) ->
    ...
    false ->
        ...
        Acc2 = lists:zipwith(fun(X, Y) -> Y ++ X end, Items2, Acc),
